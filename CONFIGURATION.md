...
This document describes the updated configuration for EHR Backend integration with the EHR Keys Management System.
...
2. Connect to the EHR Keys Management System via the `ehr-keys-net` network
...
Local development environment variables. Contains sensitive credentials.
...
- Connects to `ehr-keys-net` external network
...
The EHR Backend connects to the EHR Keys Management System via the `ehr-keys-net` network:
...
**Prerequisite**: The EHR Keys Management System must be running with the `ehr-keys-net` network.

## OpenBao Configuration

The `config/openbao.config.js` has been updated to:
- Prioritize environment variables from `.env` files
- Support multiple endpoint fallbacks:
  1. `OPENBAO_ADDR` environment variable
  2. `http://openbao:8200` (Docker network)
  3. `http://localhost:18200` (local development)
- Provide detailed logging for connection attempts
...
# For testing, env.test is already configured
```
...
#### "network ehr-keys-net declared as external, but could not be found"
...
2. **If network doesn't exist:**
   - Ensure EHR Keys Management System is running
   - Or create a local network for development:
     ```bash
     docker network create ehr-keys-net
     ```
...
2. Check if `ehr-keys-net` network exists: `docker network ls | grep ehr-keys-net`
3. Verify OpenBao is accessible: `curl http://openbao:8200/v1/sys/health`
